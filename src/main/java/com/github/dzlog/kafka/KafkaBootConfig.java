...
public class KafkaBootConfig implements InitializingBean {
    ...
    private String dataCenter = System.getProperty("dataCenter");

	private String kafkaAutoOffsetReset = KAFKA_AUTO_OFFSET_RESET;

	@Override
	public void afterPropertiesSet() throws Exception {
		String kafkaClusterCode = collectConfigService.getKafkaClusterCode();
		Assert.hasText(kafkaClusterCode, "kafkaClusterCode 不能为空");
	...
	private ConcurrentKafkaListenerContainerFactory<String, ByteBuffer> buildContainerFactory(KafkaReceiver receiver) {
        ...
        factory.setTopics(ArrayUtils.toStringArray(topics.toArray()));
	...
	@Bean("dcContainerFactory")
	public ConcurrentKafkaListenerContainerFactory<String, ByteBuffer> dcContainerFactory() {
