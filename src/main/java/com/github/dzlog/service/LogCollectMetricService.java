...
public class LogCollectMetricService {
	...
	public List<LogCollectMetric> findByNamedParam(String[] propertyNames, Object[] values) {
		try {
			String sql = "SELECT * FROM log_collect_metric";
			...
			LOGGER.error("query log_collect_metric error: {} ", e.getMessage());
			return null;
		}
	}
	...
	public int updateEntity(LogCollectMetric entity) {
		try {
			String sql = "UPDATE log_collect_metric SET " +
			...
			LOGGER.error("update log_collect_metric error: {} ", e.getMessage());
			return 0;
		}
	}
	...
	public int insertEntity(LogCollectMetric entity) {
		try {
			String sql = "insert into log_collect_metric" +
			...
			LOGGER.error("insert into log_collect_metric error: {} ", e.getMessage());
			return 0;
		}
	}
}
