...
public class LogCollectConfigService implements InitializingBean {
	...
	private String kafkaClusterCode = null;
	...
	public void afterPropertiesSet() throws Exception {
		...
		if (!"hangzhou".equals(dataCenter)) {
			throw new IllegalArgumentException("dataCenter 只能是 hangzhou");
		}
				...
				.build(
						new CacheLoader<String, LogCollectConfig>() {
							public LogCollectConfig load(String code) throws Exception {
								try {
									String sql = "SELECT * FROM log_collect_config where code = ? and kafka_cluster = ? and data_center = ?";
				...
				.build(
						new CacheLoader<String, String>() {
							public String load(String topic) throws Exception {
								try {
									String sql = "SELECT code, topic FROM log_collect_config where topic like ? and kafka_cluster = ? and data_center = ?";
	...
	public List<String> queryAllTopics() {
		String sql = "SELECT distinct kafka_topic FROM log_collect_config where run_status = 1 and kafka_cluster = ? and data_center = ?";
	...
	public List<LogCollectConfig> queryAllCollects() {
		try {
			String sql = "SELECT * FROM log_collect_config where data_center = ? and kafka_cluster = ? and run_status=1";
	...
	public String getKafkaClusterCode() {
		return kafkaClusterCode;
	}
}
